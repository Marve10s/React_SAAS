...
const PlanCard = ({ price, title, description, features, isPopular }) => {
  return (
    <div className="plan-card border flex flex-col justify-between bg-white/20 backdrop-blur-lg rounded-lg h-full p-6 hover:shadow-md text-left relative">
      {isPopular && (
        <div className="absolute -top-6 left-0 bg-indigo-500 text-white px-2 py-1 rounded-t-lg w-full text-center">
          Popular
        </div>
      )}
      <div className="">
        <div className="inlinep-flex items-end ">
      ...
      <div className="">
        <button className="bg-indigo-500 py-2 mt-3 rounded-lg text-white w-full hover:bg-indigo-600">
          Select Plan
        </button>
      </div>
    </div>
